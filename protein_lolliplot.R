...
loadVCFs <- function(path) {
  if (endsWith(path, ".rds")) {
    ...
    vcfs <- lapply(vcf_list, function(x) {
      vcf <- readVcf(x, "hg38")
...
# Args (1. gene name 2. path to vcfs or rds path)
...
chr_only <- c("chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", 
              ...
              "chrX", "chrY")
output_filename <- paste(gene, "lolliplot.pdf", sep="_")
...
# Plot
dir.create(file.path(getwd(), "lolliplot_output"), showWarnings = FALSE)
setwd(file.path(getwd(), "lolliplot_output"))
